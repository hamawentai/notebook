...
如果不想让他人获得一个pointer或reference指向某个整数常量，就可以使用enum。优秀的编译器不会为“整数型const对象”设定另外的存储空间（除非创建了一个指针/引用指向了该对象）（~~直接硬编码？~~ ->常量折叠），而**enum和#define绝不会导致非必要内存分配**。
...
RAII (资源获取就是初始化Resource Acquisition Is Initialization)，在构造函数中获得资源，并在析构函数中释放资源。
...
以内存为例，RAII会在构造函数中申请内存，在析构时自动释放内存，如果可以复制就会释放同一块内存两次。
