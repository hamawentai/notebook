...
**析构函数体自身并不直接销毁成员！**成员是在析构函数之后隐含的析构阶段中被销毁的，在整个对象销毁过程中，析构函数体是作为成员销毁步骤之外的另一部分进行的。flag
...
```cpp
// 如函数重载时
// 虽然删除函数不可被使用，但它还是程序的一部分。因此在重载决议时还是被纳入考量。所以那些无意义的调用都会被拒绝编译
...
**如果一个类有数据成员不能默认构造、拷贝、复制或销毁，则对于的成员函数将被定义为删除的。**
...
对于一个运算函数来说，**它或者是类的成员，或者至少含有一个类类型参数，如果都是内置类型，则是错误的。**：
...
`||`、`&&`：重载版本不能保留内置运算符的短路求值属性，两个运算对象总是会被求值
...
lambda通过**引用捕获**变量时，将由程序负责确保lambda执行时引用所指对象确实存在。所以，编译器可以直接使用该引用而无需在lambda产生的类中将其存储为数据成员。
...
```cpp
// 虽然只循序一步的隐式类型转换，但是这条规则与“内置类型”的转换不矛盾
ClassA a = 3.14; // 正确
```
...
改代码会调用cin(istream类)的bool类型转换运算符，那么：

cin会变为bool，bool会变为int，上述代码就会变为一个为运算的语意
...
如果某次函数调用使用了默认是惨，则该实参由本次调用的静态类型决定。
...
友元关系不能传递flag。同样，友元关系同样也不能继承，基类的友元在访问派生类成员时不具有特殊性；派生类的友元也不能随意访问基类的成员。
...
如果指针指向继承体系中的某个类型，则有可能出现指针的静态类型与被删除对象的动态类型不符合的情况。需要在基类中讲析构函数定义成虚函数以确保执行正确的析构函数版本。
